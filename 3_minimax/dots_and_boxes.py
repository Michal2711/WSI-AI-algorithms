...
class DotsAndBoxes(Game):
    ...
    def __init__(self, size: int = 2, first_player: Player = None, second_player: Player = None):
        ...
        self.state = DotsAndBoxesState(self.first_player, self.second_player, size)
...
class DotsAndBoxesState(State):
    ...
    def evaluate(self) -> float:
        scores = self.get_scores()
        # print(f' pierwszy : {self.first_player.char} , drugi : {self.second_player.char}')
        first_points = scores[self.first_player]
        second_points = scores[self.second_player]
        return float(second_points) - float(first_points)
